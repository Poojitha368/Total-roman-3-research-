...
def heuristic1(graph):
    ...
    # Check if the number of vertices allows for the desired labeling
    if len(vertices) % 3 != 1:
        labels = [2, 1, 0]
        labeling = {vertex: labels[i % 3] for i, vertex in enumerate(vertices)}
        return labeling, sum(labeling.values())
...
def genetic_algorithm(num_vertices, graph, population_size=100, mutation_rate=0.1, generations=100):
    ...
    for generation in range(generations):
        ...
        print(f"Generation {generation}: Best Fitness: {best_fitness_value}, Best Chromosome: {best_chromosome}")
...
def main():
    ...
    best_chromosome, best_fitness_value = genetic_algorithm(num_vertices, graph, population_size=100, generations=100)
